...
router.get('/similar/by-key/:id', authenticateJWT, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const list = await txCol.find({ userId: new ObjectId(user._id), keyHash: tx.keyHash }).project({ _id: 1, category: 1 }).limit(100).toArray();
...
// List first 50 user patterns for the authenticated user
...
router.post('/:id/category-apply', authenticateJWT, async (req: Request, res: Response): Promise<void> => {
    ...
    console.log('[CATEGORY-APPLY][req]', { id, category, scope, createUserPattern, color, img });
        ...
        const keyHash = base.keyHash || hashKey(rawKey);
        if (!base.keyHash) await txCol.updateOne({ _id: base._id }, { $set: { keyHash } });
        let modified = 0;
        ...
        if (scope === 'one') {
            ...
            const r = await txCol.updateMany({ userId: new ObjectId(user._id), keyHash }, { $set: commonSet });
            modified = r.modifiedCount;
        } else {
            res.status(400).json({ message: 'invalid scope' }); return; }
        // Создаём паттерн всегда (если пользователь применил категорию вручную) — даже если флаг не передали.
        try {
