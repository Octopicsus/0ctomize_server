import type { AutoCategoryFields } from './classifier';
...
const classificationCache = new Map<string, AutoCategoryFields>();
const merchantTitleCache = new Map<string, string>();

function cacheKey(title: string, description: string, txType: 'Expense' | 'Income') {
...
const CATEGORY_PROMPTS: Record<'Expense'|'Income', string> = {
...
function sanitizeModelOutput(raw: string): { category: string; confidence: number; reason: string } | null {
  try {
...
function createSystemPrompt(txType: 'Expense' | 'Income'): string {
...
export async function llmClassifyTransaction(
  title: string,
  description: string,
  txType: 'Expense' | 'Income'
): Promise<AutoCategoryFields | null> {
  ...
  try {
    ...
    const result: AutoCategoryFields = {
      category: parsed.category,
...
export async function llmClassifyTransactionsBatch(
  transactions: Array<{ title: string; description: string; txType: 'Expense' | 'Income' }>
): Promise<Array<AutoCategoryFields | null>> {
  const results: Array<AutoCategoryFields | null> = [];
...
export async function llmNormalizeMerchantTitle(rawTitle: string, description?: string): Promise<string | null> {
  const apiKey = process.env.DEEPSEEK_SECRET_KEY;
  if (!apiKey) return simpleTitleSanitize(rawTitle);
  if (!rawTitle) return null;
  const key = `norm|${rawTitle.toLowerCase()}|${(description||'').toLowerCase()}`.slice(0,400);
  if (merchantTitleCache.has(key)) return merchantTitleCache.get(key)!;
...
`;
  ...
  try {
    ...
    if (!resp.ok) return simpleTitleSanitize(rawTitle);
    const data: any = await resp.json();
    let content: string = data?.choices?.[0]?.message?.content || '';
    content = content.replace(THINK_TAGS_REGEX, '').trim();
    content = content.split('\n')[0].trim();
    if (!content) return simpleTitleSanitize(rawTitle);
    const cleaned = postSanitizeMerchant(content);
    merchantTitleCache.set(key, cleaned);
    if (merchantTitleCache.size > CACHE_MAX_ENTRIES) {
      for (const k of Array.from(merchantTitleCache.keys()).slice(0, Math.ceil(CACHE_MAX_ENTRIES*0.1))) merchantTitleCache.delete(k);
    }
    return cleaned;
  } catch {
...
function simpleTitleSanitize(t: string): string {
...
function postSanitizeMerchant(t: string): string {
