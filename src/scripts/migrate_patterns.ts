 ...
 */
import { connectDB, closeDB } from '../middleware/database';
import { buildRawKey, hashKey } from '../utils/patterns';
...
async function run() {
  ...
  while (await cursor.hasNext()) {
    ...
    if (category) entry.categoryCounts[category] = (entry.categoryCounts[category] || 0) + 1;
  ...
  for (const [keyHash, data] of acc) {
    ...
    for (const t in data.titleCounts) {
      ...
      if (c > maxTitle) { maxTitle = c; canonicalTitle = t; }
    }
    ...
    for (const c in data.categoryCounts) {
      ...
      if (cnt > maxCat) { maxCat = cnt; suggestedCategory = c; }
    }
    if (maxCat < data.total * 0.5) suggestedCategory = undefined;
...
run().catch(e => { console.error('Pattern migration failed:', e); closeDB(); process.exit(1); });
