...
async function run() {
  ...
  while (await cursor.hasNext()) {
    const grp: any = await cursor.next();
    if (!grp) break;
    groups++;
    ...
    if (created && created.length) {
      ...
      for (let i = 1; i < created.length; i++) {
        ...
        if (t < min) { min = t; keepIdx = i; }
      }
    }
    const keepId = ids[keepIdx];
    const removeIds: ObjectId[] = ids.filter((_: any, i: number) => i !== keepIdx);
    if (removeIds.length) {
...
run().catch(e => { console.error('Dedupe failed:', e); closeDB(); process.exit(1); });
